...
| **3. Cart, Wishlist, Contact & Transactional Emails** | **PARTIAL** | `Services/CartService.cs`<br>`Controllers/CartController.cs`<br>`Controllers/AccountController.cs`<br>`Controllers/HomeController.cs`<br>`Services/EmailSender.cs`<br>`wwwroot/js/site.js` | **Implemented**: Session/User cart merging, cart line operations, authenticated user wishlist toggle & paginated list with server-confirmed AJAX state, badge unhiding and dynamic count synchronization, real contact message submission with `ContactPolicy` rate limiting, honeypot bot trap, database persistence, and AJAX JSON response, email HTML generation.<br>**Missing**: No staff support inbox to view/reply to `ContactMessage`. No transactional email outbox queue or retry ledger if SMTP fails. Guest wishlist is not persisted across devices. |
